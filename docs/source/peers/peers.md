...
fundamental element of the network because they host ledgers and smart contracts
(aka chaincodes). Recall that a ledger immutably records all the transactions
...
respectively. These aspects of a peer make them a good starting point to
...
Because a peer is a *host* for smart contracts and ledgers, applications and
...
neither ledgers nor smart contracts. We'll see later how ledgers get created,
and smart contracts get installed, on peers.
...
It's worth mentioning in passing whether or not users have installed smart
contract chaincodes for use by external applications, peers also have special
**system chaincodes** that are always present. These are not discussed in detail
in this topic.

### Multiple Smart Contracts

There isn't a fixed relationship between the number of ledgers a peer has and
the number of smart contracts that can access that ledger. A peer might have
many smart contracts and many ledgers available to it.
...
We'll see a little later why a the concept of **channels** in Hyperldeger Fabric
is important when hosting multiple ledgers or multiple smart contracts on a
peer.
...
Applications always connect to peers when they need to access ledgers and smart
contracts. The Hyperledger Fabric Software Development Kit (SDK) makes this easy
for programmers -- its APIs enable applications to connect to peers, invoke
smart contracts to generate transactions, order transactions for application to
the ledger, and ledger notifications when transactions are committed to peers.
...
on every peer. In this example application A connects to P1 and invokes smart
contract S1 to query or update the ledger L1. P1 invokes S1 to generate a
transaction that contains a query result or a proposed ledger update.
Application A receives the transaction response, and for query transactions,
the process is now complete.  For update transactions, A send transactions to
O1 for ordering. O1 collects transactions from across the network into blocks,
and distributes these to all peers, including P1. P1 checks the transaction
before applying to L1. Once L1 is updated, P1 generates a notification,
received by A, to signify completion.*
...
invoke a smart contract, unlike with ledger-querying applications, an individual
peer cannot perform a ledger update at this time, because other peers must first
agree to the change -- a process called **consensus**. Therefore, peers return
to the application a **proposed** update transaction -- one that this peer would
apply subject to other peers' prior agreement. The first extra step -- four --
requires that applications send an appropriate set of these peer proposals to
the entire network of peers for approval, and subsequent update of their
...
the application is notified asynchronously, as shown in step five.
...
Blockchain networks are administered by a collection of organizations known as a
Consortium. Peers are central to how this kind of distributed network is built
because they are owned by -- and are the connection points to the network for --
these organizations.
...
identities issued by CA1. Channel C determines from its channel configuration
policy that identities from CA1 should be associated with Org1 using ORG1.MSP.
...
checked against the channel configuration policy to determine its rights as
dictated by the organization to which it belongs*. The mapping of identity to
organization is provided by a component called a *Membership Service Provider*
(MSP) -- it determines how a peer gets assigned to a specific role in a
particular organization and accordingly gains appropriate access to blockchain
resources. Moreover, a peer can only be owned by a single organization, and is
therefore associated with a single MSP. We'll learn more about peer access
control later in this topic, and there's an entire topic on MSPs and access
control policies elsewhere in this guide.  But for now, think of an MSP as
providing linkage between an individual identity and a particular organizational
...
proposed ledger transaction to the application, but do not apply the proposed
update to their copy of the ledger. In the second phase, these separate
endorsements are collected together as transactions and packaged into blocks for
distribution to peers. In the final phase, these blocks are distributed back to
every peer where each transaction is validated before being applied to that
...
agree that each would be satisfied -- in principle -- to apply a particular
...
independently executes a smart contract using the transaction proposal to
...
application has received a sufficient number of signed transaction responses,
...
transaction responses. In this example, application A1 generates transaction T1
...
Application A1 receives two endorsed responses for transaction T1, namely E1 and E2.*
...
depends on the *endorsement policy* (defined in a smart contract), which defines
...
A peer endorses a transaction by adding its digital signature, and signing the
entire payload using its private key. Endorsement of a transaction response by
an organization's peer is confirmation that this peer agrees with the
transaction result. In our example, if peer P1 is owned by organization Org1,
endorsement E1 corresponds to Org1 saying "Transaction T1 response R1 on ledger
L1 is approved by peer P1!".

Phase 1 ends when the application receives signed transaction responses from
...
different time on different peers with ledgers at a different states -- in which
case an application can simply request a more up-to-date proposal. Less likely,
but much more seriously, results might be different because the smart contract
is *non-deterministic*. Non-determinism is the enemy of smart contracts and
ledgers and if it occurs it indicates a serious problem with the proposed
...
transaction proposal responses from many applications. It orders this
...
*endorsement policy* of the smart contract which generated the transaction. For
...
organizations have agreed that the transaction can be applied to the ledger.
...
transaction may have updated the ledger such that the transaction update can no
longer be applied. In this way each peer's copy of the ledger is kept consistent
...
the blockchain network. This is often helpful as it keeps the logic of the smart
contract confidential to endorsing organizations. This is in contrast to the
output of the smart contracts (the transaction proposal responses) which are
shared to every peer in the channel, whether or not they endorsed the
...
*Chaincode* events that the smart contract execution has produced can also be
...
most fundamental element -- they form the network, host smart contracts and the
